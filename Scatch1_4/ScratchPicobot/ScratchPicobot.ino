...
#define WIFI_ACCESS_POINT_NAME                  "<Your WIFI name>"
#define WIFI_ACCESS_POINT_PASSWORD              "<WIFI Password>"
#define YOUR_COMPUTERS_IP_ADDRESS               "<IP address of your computer - e.g. 192.168.1.1>"
#define SCRATCH_UPDATE_INTERVAL_IN_MILLISECONDS  1000

////////////////////////////////////////////////////////////////
// Standard defines for Picobot2
...
void setup()
{
  ...
  sendData("AT+CWJAP=\"" + String(WIFI_ACCESS_POINT_NAME) + "\",\"" + String(WIFI_ACCESS_POINT_PASSWORD) + "\"\r\n", 30000, DEBUG); // connect to AP
  sendData("AT+CIFSR\r\n", 1000, DEBUG); // get ip address
  sendData("AT+CIPMUX=0\r\n", 1000, DEBUG); // ensure single connection ONLY
  sendData("AT+CIPSTART=\"TCP\",\"" + String(YOUR_COMPUTERS_IP_ADDRESS) + "\",42001\r\n", 1000, DEBUG);
...
// This gets called by the Arduino Operating System automatically and often
void loop()
{
  // Are there any messages from Scratch pending
  String dataraw = "";
  ...
  // Process the command sent to us from Scratch
  if (dataraw.length() > 0)
  {
    dataraw.toLowerCase();
  ...
  else
  {
    ...
    if (currentMillis - previousMillis >= SCRATCH_UPDATE_INTERVAL_IN_MILLISECONDS)
    {
